...
public class DNSResolver {
	...
	List<String> rootServers = rootServersList;
	
	public Message lookUpAddress(Message message, List<String> servers) throws UnknownHostException, IOException {
		Message res = null;
	...
	public Message resolveDNS(String hostName, int dnsType, boolean dnssec_required) throws IOException {
		...
		String[] labels = hostName.toLowerCase().split("\\.");
...
//			System.out.println(response);
			if (response != null) {
				...
				RRset[] recs = response.getSectionRRsets(Section.ANSWER);
				
				DNSSec dnsObj = new DNSSec();
				if(dnssec_required) {
					System.out.println("DNSSEC Verifcation required");
					boolean dnsVerified = dnsObj.performDNSVerification(recs, rootServers);
					if (dnsVerified) {
						System.out.println("DNSSEC verified for "+dnsString);
					}
				} else {
					System.out.println("DNSSEC not supported)");
				}
				
				//System.out.println("DNS Verification Done");
